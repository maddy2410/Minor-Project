...
with detection_graph.as_default():
    ...
    with tf.compat.v2.io.gfile.GFile(path_ckpt, 'rb') as fid:
...
def show_classes_in_window(img, response):
    ...
    print("Purple : Drop Down Menu")
    print("Green : Division")
    print("Blue : Input Box")
    print("Yellow : Video box")
    print("Orange : Text")
    print("Red : Button")
...
def detect_objects_from_image(imagePath, debug = False):
    ...
    modResponse.clear()
    aux.clear()
    # Reading Image form image path
    img = cv2.imread(imagePath)

    #converting it to greyScale
    gray = cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)

    #converting image using threshold function which basically changes color of one pixel to another if its value is greater than threshold value
